...
class DriverHomePage extends Component {
            ...
            this.setState(
              {
                ...
                startCoords: [
                  rideRequestData.customerLatitude,
                  rideRequestData.customerLongitude,
                ],
                destCoords: [
    ...
    updateDriverStatus(userId, isDriver, newStatus, () => {
      ...
      if (newStatus === 0) {
        ...
        interval = setInterval(() => {
          this.checkForNewCustomers(userId);
        }, 3000);
      }
      this.setState({
        ...
        this.setState(
          {
            ...
            currentStatus: 3,
            walkPathInterval: setInterval(() => {
              this.walkThroughPath(this.state.coordinateIndex);
            }, 250),
          },
          () => {}
        );
  ...
  walkThroughPath = coordinateIndex => {
    // driver to customer
    if (
      coordinateIndex >= this.state.coordinates.length &&
      this.state.reachedCustomer
    ) {
      ...
      coordinateIndex >= this.state.coordinates.length &&
      this.state.reachedCustomer === false
    ) {
        ...
        {this.state.allowedLocation && (
          <div>
            {!this.state.foundRider && !this.state.rideComplete && (
              <div>
                <h1>Welcome back!</h1>
                To begin accepting rides or disable requests, update your status
                below.
                <div style={SPACER} />
                <Col>
                  <Dropdown isOpen={this.state.isOpen} toggle={this.toggle}>
                    ...
                    <DropdownMenu>
                      <DropdownItem value={0} onClick={this.selectStatus}>
                        Not Available
                      </DropdownItem>
                      <DropdownItem value={1} onClick={this.selectStatus}>
                        Available
                      </DropdownItem>
                    </DropdownMenu>
                  </Dropdown>
                ...
                <div style={SPACER} />
                <Col>
                  <Button color="primary" onClick={this.updateLocation}>
                    Update Location
                  </Button>
                  <div style={SPACER} />
                ...
                {this.state.hasAccepted === null && (
                  <div>
                    ...
                    <RideRequestInformation
                      ...
                      customerLong={
                        this.state.rideRequestInfo.customerLongitude
                      }
                      destinationLat={
                        this.state.rideRequestInfo.destinationLatitude
                      }
                      destinationLong={
                        this.state.rideRequestInfo.destinationLongitude
                      }
                      accept={this.accept}
            ...
            {this.state.rideComplete && <BookingPayment isDriver />}
          </div>
        )}
