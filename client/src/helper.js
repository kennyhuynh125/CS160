...
export const isCreditCardValid = (ccNumber, ccType) => {
  ...
  switch (ccType) {
    case 'visa':
      ...
      break;
    case 'mastercard':
      const number = parseInt(ccNumber.substring(0, 2), 10);
    ...
    case 'discover':
      ...
      const typeOne = parseInt(ccNumber.substring(0, 7), 10);
      const typeTwo = parseInt(ccNumber.substring(0, 4), 10);
      const typeThree = parseInt(ccNumber.substring(0, 1), 10);
      const typeFour = parseInt(ccNumber.substring(0, 4), 10);
      let isValid = false;
      ...
      if (isValid === false) {
        results +=
          'Discover cards must start with 6011, 65, or be between 62216-622925 or 644-649';
      }
      break;
    default:
      results += '';
